...
class _DeleteAccountState extends State<DeleteAccount> {
  ...
  Future<void> _deleteUserData(String uid) async {
    try {
      // Delete posts
      QuerySnapshot postsSnapshot = await _firestore.collection('posts').where('userId', isEqualTo: uid).get();
                ...
                children: [
                  Expanded(
                    child: Column(
                      ...
                      children: [
                       userViewModel.isGoogleSignup==true?  IconButton(onPressed: () { 
                        }, icon: const Icon(Icons.delete_outline,color: redColor,size: 60,),):const SizedBox(),
                        text("Delete Account",
                        ...
                        text(
                          ...
                          textColor: Colors.grey, maxLine: 5, isCentered: true,
                        ).paddingBottom(20),
                        const SizedBox(
                          height: 4,
                        ),
                    userViewModel.isGoogleSignup==true?const SizedBox(): Padding(
